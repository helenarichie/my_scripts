...
T_min, T_max = 5e3, 1e8
d_min, d_max = 5e-29, 8e-24  # g cm^-3
###############################################################################
...
t = ax.text(-24.33+0.12-0.05, 4.12+.48, "cloud", fontsize=25, zorder=10)
t.set_bbox(dict(facecolor='white', edgecolor="white"))
################################################################################

cgols_ns = np.array([1.5e-2, 2e-2, 9e-3, 4e-3, 2.1e-3, 1.5e-3, 1e-3, 7.5e-4, 5.5e-4, 4e-4, 3e-4, 2.5e-4])
log_cgols_Ts = [7, 7.375, 6.375, 6.4, 6.5, 6.575, 6.625, 6.7, 6.75, 6.875, 7, 7.25]
...
cgols_5kpc  = [cgols_ns[7], log_cgols_Ts[7]]
cgols_10kpc = [cgols_ns[11], log_cgols_Ts[11]]

##################################### Base ####################################
cgols = ax.scatter(np.log10((cgols_base[0])*0.6*MP), cgols_base[1], marker="x", c="coral", s=100, linewidths=3.5, zorder=10)
t = ax.text(-26-0.05-0.05, 7.1, "r = 0.1 kpc", fontsize=25, zorder=10)
t.set_bbox(dict(facecolor='white', edgecolor="white"))
################################################################################

##################################### 5 kpc ####################################
ax.scatter(np.log10((cgols_5kpc[0])*0.6*MP), cgols_5kpc[1], marker="x", c="coral", s=100, linewidths=3.5, zorder=10)
t = ax.text(-27.43-0.1, 6.42, "r = 5 kpc", fontsize=25, zorder=10)
t.set_bbox(dict(facecolor='white', edgecolor="white"))
################################################################################

##################################### 10 kpc ###################################
ax.scatter(np.log10((cgols_10kpc[0])*0.6*MP), cgols_10kpc[1], marker="x", c="coral", s=100, linewidths=3.5, zorder=10)
...
ax.legend([cgols], ["CGOLS"], loc="upper left", fontsize=25)
