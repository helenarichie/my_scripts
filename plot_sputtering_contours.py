...
T_min, T_max = 5e3, 1e8
d_min, d_max = 2e-28, 8e-22  # g cm^-3
###############################################################################
...
for j, tau in enumerate(tau_sp):
        ax.plot(np.log10(d_sput[j]), np.log10(T_sput[j]), linestyle="--", linewidth=1.5, color="black", label=r'$10^{{{:d}}}$'.format(a[j]), zorder=1)
...
ax.scatter(np.log10((10**2)*0.6*MP), 4, marker="x", c="slateblue", s=100, linewidths=3.5, zorder=10)
t = ax.text(-22.32, 4.12, "cloud", fontsize=25, zorder=10)
t.set_bbox(dict(facecolor='white', edgecolor="white"))
ax.scatter(np.log10((10**-2)*0.6*MP), 7.5, marker="x", c="steelblue", s=100, linewidths=3.5, zorder=10)
t = ax.text(-26.47, 7.06, "starburst\n   wind", fontsize=25, zorder=10)
#t.set_bbox(dict(facecolor='white', edgecolor="white"))
