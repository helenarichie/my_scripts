...
for i in range(ns, ne+1):
    ...
    # plot
    fig, axs = plt.subplots(nrows=2, ncols=1, figsize=(25,9))

    if vlims:
        im = axs[0].imshow(np.log10(d_gas[0].T), origin="lower", vmin=vlims_gas[0], vmax=vlims_gas[1], extent=[0, nx*dx, 0, ny*dx])
    else:
        im = axs[0].imshow(np.log10(d_gas[0].T), origin="lower", extent=[0, nx*dx, 0, ny*dx])
    ...
    axs[0].set_title(r"Gas Density Slice", fontsize=fontsize)
    axs[0].set_xlabel(r"$x~$[{}]".format(unit), fontsize=fontsize)
    axs[0].set_ylabel(r"$y~$[{}]".format(unit), fontsize=fontsize)
    axs[0].text(spacing, 0.1*dx*ny, f'{round(t_arr[0]/1e6, 2)} Myr', color='white', fontsize=fontsize)
    ...
    if vlims:
        im = axs[1].imshow(np.log10(d_dust[0].T), origin="lower", cmap="plasma", vmin=vlims_dust[0], vmax=vlims_dust[1], extent=[0, nx*dx, 0, ny*dx])
    else:
        im = axs[1].imshow(np.log10(d_dust[0].T), origin="lower", cmap="plasma", extent=[0, nx*dx, 0, ny*dx])
    ...
    # plot and save
    plt.savefig(pngdir + f"{i}_density_slice.png", dpi=300)
    plt.close()
    ...
    if vlims:
        im = axs[0].imshow(np.log10(T[0].T), origin="lower", cmap="inferno", vmin=vlims_T[0], vmax=vlims_T[1], extent=[0, nx*dx, 0, ny*dx])
    else:
        im = axs[0].imshow(np.log10(T[0].T), origin="lower", cmap="inferno", extent=[0, nx*dx, 0, ny*dx])
    ...
    axs[0].set_yticks(np.arange(0, ny*dx, spacing))
    axs[0].tick_params(axis='both', which='both', direction='in', color='black', top=1, right=1, length=8)
    axs[0].set_title(r"Temperature Slice", fontsize=fontsize)
    axs[0].set_xlabel(r"$x~$[{}]".format(unit), fontsize=fontsize)
    axs[0].set_ylabel(r"$y~$[{}]".format(unit), fontsize=fontsize)
    ...
    # xy pressure
    if vlims:
        im = axs[1].imshow(np.log10(p_gas[0].T/KB), origin="lower", cmap="magma", vmin=vlims_p[0], vmax=vlims_p[1], extent=[0, nx*dx, 0, ny*dx])
    else:
        im = axs[1].imshow(np.log10(p_gas[0].T/KB), origin="lower", cmap="magma", extent=[0, nx*dx, 0, ny*dx])
    ...
    axs[1].set_yticks(np.arange(0, ny*dx, spacing))
    axs[1].tick_params(axis='both', which='both', direction='in', color='black', top=1, right=1, length=8)
    axs[1].set_title(r"Gas Pressure Slice", fontsize=fontsize)
    axs[1].set_xlabel(r"$x~$[{}]".format(unit), fontsize=fontsize)
    axs[1].set_ylabel(r"$y~$[{}]".format(unit), fontsize=fontsize)

    # xy velocity slice
    if vlims:
        im = axs[2].imshow(vx[0].T*1e-5, origin="lower", vmin=vlims_v[0], vmax=vlims_v[1], extent=[0, nx*dx, 0, ny*dx])
    else:
        im = axs[2].imshow(vx[0].T*1e-5, origin="lower", extent=[0, nx*dx, 0, ny*dx])
    ylabel = r'$v_x$ [km/s]'
    ...
    axs[2].set_ylabel(r"$y~$[{}]".format(unit), fontsize=fontsize)

    fig.tight_layout()
    
    # plot and save
    plt.savefig(pngdir + f"{i}_PvT_slice.png", dpi=300)
    plt.close()
