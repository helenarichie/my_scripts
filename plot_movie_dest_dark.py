...
pngdir = os.path.join(basedir, "png/movie_dark")
...
for i in range(ns, ne+1):
    ...
    # plot
    fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(34,9), gridspec_kw={"width_ratios":[4, 25], 'wspace':0, 'hspace':0})
    ...
    ax[0][0].tick_params(axis='both', which='both', direction='in', color='white', labelbottom=0, top=1, right=1, length=ticklength, width=tickwidth)
    ax[0][1].tick_params(axis='both', which='both', direction='in', color='white', labelleft=0, labelbottom=0, top=1, right=1, length=ticklength, width=tickwidth)
    ax[1][0].tick_params(axis='both', which='both', direction='in', color='white', top=1, right=1, length=ticklength, width=tickwidth)
    ...
    ax[1][0].plot(time_i/1e3, (sputter_tot_i+sputter_tot_hot_i)/mass_dust_init, c="white", label=r"sputtered", linewidth=linewidth, zorder=1)
    ax[1][0].plot(time_i/1e3, sputter_tot_i/mass_dust_init, c="white", linestyle="--", linewidth=linewidth-2, zorder=1)
    ax[1][0].plot(time_i/1e3, sputter_tot_hot_i/mass_dust_init, c="white", linestyle="-.", linewidth=linewidth-2, zorder=1)

    mass_cl_init = mass_cloud[0]
    mass_cloud_i = mass_cloud[time<=t]
    mass_out_cloud_i = mass_out_cloud[time_output<=t]

    ax[0][0].plot(time_i/1e3, mass_cloud_i/mass_cl_init, linewidth=linewidth, c="#49b4ab")
    ...
    plt.savefig(os.path.join(pngdir, f"{i}_movie_dark.png"), bbox_inches="tight", dpi=300)
    plt.close()
