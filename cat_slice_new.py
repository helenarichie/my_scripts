...
for n in range(ns, ne+1):
  ...
  for i in range(0, n_procs):
    ...
    if SCALAR:
      scalar_xy[xs:xs+nxl,ys:ys+nyl] += filein['basic_scalar_xy']
      scalar_xz[xs:xs+nxl,zs:zs+nzl] += filein['basic_scalar_xz']
      scalar_yz[ys:ys+nyl,zs:zs+nzl] += filein['basic_scalar_yz']
  ...
  if SCALAR:
    fileout.create_dataset('basic_scalar_xy', data=scalar_xy)
    fileout.create_dataset('basic_scalar_xz', data=scalar_xz)
    fileout.create_dataset('basic_scalar_yz', data=scalar_yz)
